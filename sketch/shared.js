...
let WATER_COORDINATES_UNIT = 1;

let INITIAL_SPECTRUM_UNIT = 0,
  SPECTRUM_UNIT = 1,
...
let UI_COLOR = 'rgb(52, 137, 189)';

let PROFILE_AMPLITUDE = 50,
  PROFILE_OMEGA = 0.05,
  PROFILE_PHI = -2.5,
  PROFILE_STEP = 5,
  PROFILE_OFFSET = 52,
...
let WIND_SPEED_DECIMAL_PLACES = 1,
  SIZE_DECIMAL_PLACES = 0,
  CHOPPINESS_DECIMAL_PLACES = 1;

let SENSITIVITY = 1.0;

let WIND_SPEED_X = -1350;
let MIN_WIND_SPEED_Z = 600,
  WIND_SPEED_OFFSET = 30;

let OVERLAY_DIV_ID = 'overlay',
...
let addToVector = function (out, a, b) {
  out[0] = a[0] + b[0];
  out[1] = a[1] + b[1];
  out[2] = a[2] + b[2];
  return out;
...
let subtractFromVector = function (out, a, b) {
  out[0] = a[0] - b[0];
  out[1] = a[1] - b[1];
  out[2] = a[2] - b[2];
  return out;
...
let multiplyVectorByScalar = function (out, v, k) {
  out[0] = v[0] * k;
  out[1] = v[1] * k;
  out[2] = v[2] * k;
  return out;
...
let makeIdentityMatrix = function (matrix) {
  matrix[0] = 1.0;
  matrix[1] = 0.0;
  matrix[2] = 0.0;
  matrix[3] = 0.0;
...
let makeXRotationMatrix = function (matrix, angle) {
  matrix[0] = 1.0;
  matrix[1] = 0.0;
  matrix[2] = 0.0;
  matrix[3] = 0.0;
...
let makeYRotationMatrix = function (matrix, angle) {
...
let distanceBetweenVectors = function (a, b) {
  let xDist = b[0] - a[0],
    yDist = b[1] - a[1],
    zDist = b[2] - a[2];
...
let lengthOfVector = function (v) {
  let x = v[0], y = v[1], z = v[2];
  return Math.sqrt(x * x + y * y + z * z);
};

let setVector4 = function (out, x, y, z, w) {
  out[0] = x;
  out[1] = y;
  out[2] = z;
  out[3] = w;
...
let projectVector4 = function (out, v) {
  let reciprocalW = 1 / v[3];
...
let transformVectorByMatrix = function (out, v, m) {
...
let invertMatrix = function (out, m) {
  let m0 = m[0], m4 = m[4], m8 = m[8], m12 = m[12],
...
let premultiplyMatrix = function (out, matrixA, matrixB) {
...
let makePerspectiveMatrix = function (matrix, fov, aspect, near, far) {
...
let epsilon = function (x) {
  return Math.abs(x) < 0.000001 ? 0 : x;
};

let toCSSMatrix = function (m) { //flip y to make css and webgl coordinates consistent
  return 'matrix3d(' +
    epsilon(m[0]) + ',' +
    -epsilon(m[1]) + ',' +
    epsilon(m[2]) + ',' +
...
let setPerspective = function (element, value) {
  element.style.WebkitPerspective = value;
  element.style.perspective = value;
};

let setTransformOrigin = function (element, value) {
  element.style.WebkitTransformOrigin = value;
  element.style.transformOrigin = value;
};

let setTransform = function (element, value) {
  element.style.WebkitTransform = value;
  element.style.transform = value;
};

let setText = function (element, value, decimalPlaces) {
...
let hasWebGLSupportWithExtensions = function (extensions) {
