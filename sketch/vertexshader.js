/* Implemented water using webgl and vertex shader. */
...
let Simulator = function (canvas, width, height) {
  canvas.width = width;
  ...
  let oceanProgram = buildProgramWrapper(gl,
    buildShader(gl, gl.VERTEX_SHADER, WATER_VERTEX_SOURCE),
    buildShader(gl, gl.FRAGMENT_SHADER, WATER_FRAGMENT_SOURCE), {
      'a_position': 0,
      'a_coordinates': WATER_COORDINATES_UNIT
    });
  ...
  gl.uniform3f(oceanProgram.uniformLocations['u_oceanColor'], WATER_COLOR[0], WATER_COLOR[1], WATER_COLOR[2]);
  ...
  gl.vertexAttribPointer(WATER_COORDINATES_UNIT, 2, gl.FLOAT, false, 5 * SIZE_OF_FLOAT, 3 * SIZE_OF_FLOAT);
  ...
  this.render = function (deltaTime, projectionMatrix, viewMatrix, cameraPosition) {
    ...
    gl.enableVertexAttribArray(WATER_COORDINATES_UNIT);
    ...
    gl.disableVertexAttribArray(WATER_COORDINATES_UNIT);
